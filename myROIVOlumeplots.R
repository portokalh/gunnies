...
roinames <- read.csv('./volumes_transp_exvivo1.csv',nrows=1,header = FALSE)
volume <- read.csv('./volumes_transp_exvivo1.csv',header = TRUE)
n<-nrow(volume);
...
labs(title="Volume (% brain)",x="Genotype", y = res[i+3])


p+labs(y="Volume (% brain)",x="Group", title = res[i+3]) + theme_classic()+theme(text = element_text(size = 20)) 

ggsave(paste(pathout,res[i+3],"Vol.png"), width=5, height=4, dpi=200)
