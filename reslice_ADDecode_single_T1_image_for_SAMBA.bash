...
for bxh in $(ls ${upper_dir}/*${runno}/*_0??.bxh);do
	...
	if (($test));then
		...
		if [[ ! -e ${SAMBA_input} ]];then
			...
			t_vol1="${temp_dir}/${runno}_DWI_target.nii.gz";
					...
					if [[ ! -f ${better_header} ]];then
						if [[ ! -f ${reslice_out} ]];then
							if [[ ! -f ${tvol_1} ]];then            
								...
								#Average over 4D volume to make a quick reg target.
								fslmaths -dt 'short' ${biac_dwi} -Tmean ${t_vol1} -odt 'short'
								
							fi
							xform_prefix=S${runno}_T1_to_mean_DWI_;
							xform="${xform_prefix}0GenericAffine.mat";
							${ANTSPATH}/antsRegistration  --float -d 3 -v  -m Mattes[ ${target_vol},${qial_T1},1,32,regular,0.3 ] -t Affine[0.05] -c [ 100x100x100,1.e-7,15 ] -s 2x1x0.5vox -f 4x2x1 -u 1 -z 1 -o ${xform_prefix};
							
							
							# Reslice T1 into that first DWI volume:
							echo antsApplyTransforms -v 1 -d 3  -i ${qial_T1} -r ${t_vol1} -n BSpline -t ${xform} -o ${reslice_out};
							antsApplyTransforms -v 1 -d 3  -i ${qial_T1} -r ${t_vol1} -n BSpline -t ${xform} -o ${reslice_out} ;
						fi
